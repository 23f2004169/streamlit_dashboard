...
# Convert work_date to datetime just in case
df_progress["work_date"] = pd.to_datetime(df_progress["work_date"])

##### Filter by selected day type
if day_type == "Weekdays Only":
    df_progress = df_progress[df_progress["work_date"].dt.weekday < 5]  # 0-4 = Mon-Fri
elif day_type == "Weekends Only":
    df_progress = df_progress[df_progress["work_date"].dt.weekday >= 5]  # 5-6 = Sat/Sun
